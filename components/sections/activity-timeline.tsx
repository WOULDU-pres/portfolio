    ...
    <section id="experience" className="mx-auto max-w-6xl px-6 pb-20">
      ...
      <Card className="px-5 py-2 sm:px-8">
        {rows.map((a, i) => (
          <div
            key={`${a.org}-${a.topic}-${i}`}
            className="flex flex-col gap-2 border-b py-4 last:border-0 sm:flex-row sm:items-center sm:gap-4"
          >
            <div className="w-24 shrink-0 font-mono text-xs text-brand tabular-nums">
              {a.date}
            </div>
      ...
      <p className="mt-4 font-mono text-xs text-muted-foreground">
        * 일부 항목은 일정명 기준이며 실제 강의 완료 여부는 확인이 필요합니다.
      </p>
    </section>
  )
}
